      ...
      addToCart: (product, quantity, variantData) => {
        ...
        if (existingItem) {
          ...
          const price = variantData 
            ? (product.variants?.find(v => v._id === variantData.variantId)?.discountPrice || 
               product.variants?.find(v => v._id === variantData.variantId)?.price || 
